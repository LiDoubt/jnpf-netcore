...
    /// </summary>
    ...
    public class OrderService : IDynamicApiController, ITransient
    {
        ...
        /// </summary>
        public OrderService(ISqlSugarRepository<OrderEntity> orderRepository, ISqlSugarRepository<OrderEntryEntity> orderEntryRepository, ISqlSugarRepository<OrderReceivableEntity> orderReceivableRepository, IUserManager userManager, IUsersService usersService, IFlowTaskService flowTaskService, IFlowTaskRepository flowTaskRepository, IFlowEngineService flowEngineService, IAuthorizeService authorizeService)
        {
        ...
        public async Task<dynamic> GetList([FromQuery] OrderListQuery input)
        {
            ...
            {
                authorizeWhere = await _authorizeService.GetConditionAsync<OrderListOutput>("F4FCA43A-B563-4149-AB16-486589CFC154");
            }
            var pageList = await _orderRepository.Context.Queryable<OrderEntity, UserEntity, FlowTaskEntity>((a, b, c) => new JoinQueryInfos(JoinType.Left, a.CreatorUserId == b.Id, JoinType.Left, a.Id == c.Id)).Where((a, b) => a.DeleteMark == null).Select((a, b, c) => new OrderListOutput()
            {
            ...
            }).MergeTable().Where(queryWhere).Where(authorizeWhere).OrderBy(a => a.creatorTime, OrderByType.Desc).ToPagedListAsync(input.currentPage, input.pageSize);
        ...
        {
            var data = await _orderEntryRepository.Entities.Where(x => x.OrderId == id).OrderBy(x => x.SortCode).ToListAsync();
        ...
        {
            var data = await _orderReceivableRepository.Entities.Where(x => x.OrderId == id).OrderBy(o => o.SortCode).ToListAsync();
        ...
        {
            var data = (await _orderRepository.FirstOrDefaultAsync(x => x.Id == id && x.DeleteMark == null)).Adapt<OrderInfoOutput>();
            data.collectionPlanList = (await _orderReceivableRepository.Entities.Where(x => x.OrderId == id).ToListAsync()).Adapt<List<CollectionPlanModel>>();
            data.goodsList = (await _orderEntryRepository.Entities.Where(x => x.OrderId == id).ToListAsync()).Adapt<List<GoodsModel>>();
        ...
        public async Task Create([FromBody] OrderCrInput input)
        {
            ...
            await _orderEntryRepository.InsertAsync(orderEntryList);
            await _orderReceivableRepository.InsertAsync(orderReceivableList);
            await _orderRepository.Context.Insertable(orderEntity).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
            if (input.status == 0)
            {
        ...
        public async Task Update(string id, [FromBody] OrderUpInput input)
        {
            ...
            await _orderEntryRepository.InsertAsync(orderEntryList);
            await _orderReceivableRepository.InsertAsync(orderReceivableList);
            await _orderRepository.Context.Updateable(orderEntity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        ...
        {
            var entity = await _orderRepository.FirstOrDefaultAsync(x => x.Id == id && x.DeleteMark == null);
            if (entity != null)
            {
                await _orderEntryRepository.DeleteAsync(x => x.OrderId == id);
                await _orderReceivableRepository.DeleteAsync(x => x.OrderId == id);
                await _orderRepository.Context.Updateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        ...
        private async Task FlowSubmit(string id, FlowEngineEntity flowEngineEntity)
        {
            try
            {
                ...
                var orderEntity = await _orderRepository.FirstOrDefaultAsync(x => x.Id == id && x.DeleteMark == null);
                #region 业务信息
        ...
        private async Task FlowRevoke(FlowTaskEntity flowTaskEntity, string flowHandleModel)
        {
            ...
            {
                DbScoped.SugarScope.BeginTran();
                var orderEntity = await _orderRepository.FirstOrDefaultAsync(x => x.Id == flowTaskEntity.ProcessId && x.DeleteMark == null);
                #region 业务信息
