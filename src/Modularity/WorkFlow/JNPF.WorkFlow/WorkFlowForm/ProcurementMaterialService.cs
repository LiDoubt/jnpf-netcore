...
using JNPF.WorkFlow.Entitys.Dto.WorkFlowForm.ProcurementMaterial;
...
    /// </summary>
    ...
    public class ProcurementMaterialService : IProcurementMaterialService, IDynamicApiController, ITransient
    {
        ...
        public ProcurementMaterialService(ISqlSugarRepository<ProcurementMaterialEntity> sqlSugarRepository, ISqlSugarRepository<ProcurementEntryEntity> sqlItemSugarRepository, IBillRullService billRuleService, IFlowTaskService flowTaskService)
        {
        ...
        {
            var data = (await _sqlSugarRepository.FirstOrDefaultAsync(x => x.Id == id)).Adapt<ProcurementMaterialInfoOutput>();
            data.entryList = (await _sqlItemSugarRepository.Entities.Where(x => x.ProcurementId == id).ToListAsync()).Adapt<List<EntryListItem>>();
            return data;
        }
        #endregion
        ...
        /// <param name="itemList"></param>
        /// <returns></returns>
        private async Task Submit(string id, ProcurementMaterialEntity entity, List<ProcurementEntryEntity> itemList)
        {
            try
            {
                DbScoped.SugarScope.BeginTran();
                ...
                #region 流程信息
                await _flowTaskService.Submit(id, entity.FlowId, entity.Id, entity.FlowTitle, entity.FlowUrgent, entity.BillNo, entity.Adapt<ProcurementMaterialUpInput>(), 0, 0, true);
                #endregion
