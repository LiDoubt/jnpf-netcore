...
namespace JNPF.DataEncryption
{
    ...
    public class JWTEncryption
    {
        ...
        internal static JWTSettingsOptions SetDefaultJwtSettings(JWTSettingsOptions options)
        {
            ...
            {
                options.ValidAudience ??= "powerby JNPF";
            }
        ...
        internal static Assembly GetFrameworkContext(Assembly callAssembly)
        {
            ...
            var jnfpAssemblyName = callAssembly.GetReferencedAssemblies()
                                                       .FirstOrDefault(u => u.Name == "JNPF")
                                                       ?? throw new InvalidOperationException("No `JNPF` assembly installed in the current project was detected.");

            // 加载 JNPF 程序集
            var jnpfAssembly = AssemblyLoadContext.Default.LoadFromAssemblyName(jnfpAssemblyName);

            // 获取 JNPF.App 静态类
            FrameworkApp = jnpfAssembly.GetType("JNPF.App");
